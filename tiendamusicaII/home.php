<?php
	...
	if(isset($_POST['crear_disco'])){
			$auth_usuario->redirect('formulario-disco.php');
		}
	if(isset($_POST['desconectar']))
	{
		$logout_usuario = new USUARIO();
		if($logout_usuario->is_loggedin()!="")// esta conectado
		{
			$logout_usuario->redirect('home.php');
		}
		$logout_usuario->doLogout();
		$logout_usuario->redirect('index.php');
	}
	...
	<body>
			...
			<header>
				...
				<h1><img src="songore.png" width='50' height='50' />Songore</h1>
						...
						<form method="post" class="bienvenida">
						<label for="desconectar">Bienvenido - <?php print($userRow['nick']); ?></label>
						<?php
								if('admin'==$userRow['nick']){
								?><input type = "submit"  name="crear_disco" value = "Crear disco"/>
						<?php		}
						 ?>
